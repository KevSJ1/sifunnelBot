...
_PROMPT_TEXT = {
    "es": {
        ...
        "templates": "Plantillas de respuesta disponibles",
        "templates_rule": "Usa estas plantillas cuando encajen con la intención del cliente. Personaliza las variables entre llaves y no menciones que estás usando una plantilla.",
        "contact": "Contacto",
        "contact_name": "Nombre",
    ...
    "en": {
        ...
        "templates_rule": "Use these templates when they fit the customer's intent. Personalize variables between braces and do not mention that you are using a template.",
        "contact": "Contact",
        "contact_name": "Name",
        "contact_phone": "Phone",
        "contact_email": "Email",
...
def build_system_prompt(agent: Agent, knowledge_text: str) -> str:
    ...
    ai_templates = [item for item in getattr(client, "canned_responses", []) if item.ai_enabled]
    if ai_templates:
        templates = "\n\n".join(
            f"**/{item.shortcut}:** {item.content}" for item in sorted(ai_templates, key=lambda item: item.shortcut)
        )
        knowledge.append(_section(text["templates"], templates + "\n\n" + text["templates_rule"], 3))
    if knowledge:
