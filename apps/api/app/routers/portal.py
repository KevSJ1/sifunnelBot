...
def portal_create_canned_response(
    ...
    canned = CannedResponse(client_id=client.id, shortcut=payload.shortcut, content=payload.content.strip(), ai_enabled=payload.ai_enabled)
    db.add(canned)
    db.commit()
    db.refresh(canned)
    return canned
...
def portal_update_canned_response(
    ...
    if payload.ai_enabled is not None:
        canned.ai_enabled = payload.ai_enabled
    db.commit()
    db.refresh(canned)
    return canned
