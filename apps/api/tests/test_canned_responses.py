...
def test_saved_replies_are_managed_per_business(authenticated_client: TestClient):
    ...
    saved = created.json()
    assert saved["shortcut"] == "saludo" and "{contact_name}" in saved["content"] and saved["ai_enabled"] is False
    ...
    edited = client.patch(f"{base}/{saved['id']}", json={"content": "Hola {contact_name}!", "ai_enabled": True})
    assert edited.status_code == 200 and edited.json()["content"] == "Hola {contact_name}!" and edited.json()["ai_enabled"] is True
