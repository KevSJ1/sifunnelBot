...
/** Templates for the composer: typing "/" or pressing the template button
 * opens a picker, and selecting one inserts the rendered reply. */
export function useCannedReplies({ slug, vars, onInsert }: { slug: string; vars: CannedVars; onInsert: (text: string) => void }) {
  ...
  const open = useCallback(() => { setQuery(""); setIndex(0); }, []);
  ...
  const popup = <>
    {query === null && <button type="button" className="icon-button template-trigger" onClick={open} title={t("portal.canned.templatesTitle")} aria-label={t("portal.canned.templatesTitle")}><MessageSquareText size={17} /></button>}
    {query !== null && <div className="canned-popup">
  ...
  return { popup, onChange, onKeyDown, reset, open };
}
...
export function CannedRepliesView({ slug, canManage }: { slug: string; canManage: boolean }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const aiEnabled = data.get("ai_enabled") === "on";
      const payload = { ...body, ai_enabled: aiEnabled };
      if (editing === "new") await api<CannedResponse>(`/portal/${slug}/canned-responses`, { method: "POST", body: JSON.stringify(payload) });
      else await api<CannedResponse>(`/portal/${slug}/canned-responses/${editing.id}`, { method: "PATCH", body: JSON.stringify(payload) });
      setEditing(null);
      await load();
    } catch (err) {
    ...
    <div className="form-fields">
      {editing ? (
        <form className="modal-form canned-editor" onSubmit={save}>
          ...
          <label className="switch-row"><input type="checkbox" name="ai_enabled" defaultChecked={editing !== "new" && editing.ai_enabled} /><span><strong>{t("portal.canned.form.aiEnabled")}</strong><small>{t("portal.canned.form.aiEnabledHelp")}</small></span></label>
          ...
          <div className="canned-manage-list">
            {items.map((item) => (
              <div key={item.id} className="canned-manage-row">
                <div><strong>/{item.shortcut}{item.ai_enabled && <em className="mini-badge ai">{t("portal.canned.aiBadge")}</em>}</strong><small>{item.content}</small></div>
