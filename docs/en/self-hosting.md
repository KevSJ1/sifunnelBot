...
Only the gateway is meant to be public. For HTTPS, use the included VPS
override or put your own reverse proxy in front of it (see [Go to production](#go-to-production-https)). One instance =
...
For a single VPS, the repository includes `docker-compose.vps.yml` and a Caddy
configuration that obtains and renews certificates automatically. Point your
DNS `A`/`AAAA` record at the server first, then prepare the environment:

```bash
./scripts/generate-docker-env.sh
openssl rand -hex 32  # use separate values for SECRET_KEY and ENCRYPTION_KEY
```

Edit `.env.docker` and set `SIFUNNEL_DOMAIN`, `FRONTEND_URL=https://your.domain`,
`COOKIE_SECURE=true`, and strong unique values for `SECRET_KEY`,
`ENCRYPTION_KEY`, `POSTGRES_PASSWORD` and `WHATSAPP_BRIDGE_TOKEN`. Keep
`BIND_HOST=127.0.0.1`; the VPS override removes the API and PostgreSQL host
ports and publishes only Caddy:
...
Open `https://your.domain`. The first registration creates the agency admin.
Allow inbound TCP `80` and `443` in the VPS firewall. Keep the `.env.docker`
file private and never change `ENCRYPTION_KEY` after storing provider keys.

Stop the stack without deleting data with `make vps-down`. Caddy certificates
are stored in the `caddy_data` volume.
