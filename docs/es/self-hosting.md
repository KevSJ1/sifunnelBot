...
Solo el gateway está pensado para ser público. Para HTTPS, usa el override para
VPS incluido o pon tu propio proxy inverso delante (mira [Pasar a producción](#pasar-a-producción-https)). Una
...
### Despliegue recomendado en un VPS

Para un VPS único, el repositorio incluye `docker-compose.vps.yml` y una
configuración de Caddy que obtiene y renueva certificados automáticamente.
Apunta primero el registro DNS `A`/`AAAA` al servidor y prepara el entorno:

```bash
./scripts/generate-docker-env.sh
openssl rand -hex 32  # usa valores distintos para SECRET_KEY y ENCRYPTION_KEY
```

Edita `.env.docker` y configura `SIFUNNEL_DOMAIN`,
`FRONTEND_URL=https://tu.dominio`, `COOKIE_SECURE=true` y valores fuertes y
únicos para `SECRET_KEY`, `ENCRYPTION_KEY`, `POSTGRES_PASSWORD` y
`WHATSAPP_BRIDGE_TOKEN`. Mantén `BIND_HOST=127.0.0.1`; el override de VPS
quita los puertos públicos de API y PostgreSQL y publica solo Caddy:

```bash
make vps-up
make vps-logs SERVICE=proxy
```

Abre `https://tu.dominio`. El primer registro crea el administrador de la
agencia. Permite TCP entrante `80` y `443` en el firewall del VPS. Mantén
`.env.docker` privado y nunca cambies `ENCRYPTION_KEY` después de guardar claves
de proveedores.

Detén el stack sin borrar datos con `make vps-down`. Los certificados de Caddy
se guardan en el volumen `caddy_data`.
